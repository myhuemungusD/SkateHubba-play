...
> **Naming note:** The user-facing copy says "referee" everywhere, but the schema keeps the original `judge*` field names to avoid a Firestore migration for in-flight games. See the `[1.1.0]` entries in [`CHANGELOG.md`](../CHANGELOG.md) for the full rationale.
...
- Lobby: three parallel `onSnapshot` queries — `player1Uid == uid`, `player2Uid == uid`, `judgeId == uid` — merged and deduped in memory (`games.subscriptions.ts`)
- Gameplay: single-document listener — `onSnapshot(doc("games/{gameId}"))`
- History and the cron sweeps use compound queries backed by the composite indexes listed under [Firestore Indexes](#firestore-indexes)
...
Reserved for server-written billing-alert records. No client access. Nothing writes to this collection today — the only deployed Cloud Function is the maintainer-approved stats close-out (`functions/src/applyGameStats.ts`), which touches `games` and `users` only — so the rules lock it down against any future use.
...
| `disputes`      | `status ASC, moderationStatus ASC, createdAt DESC, __name__ DESC`       | Community dispute lane feed                                   |
| `games`         | `status ASC, turnDeadline ASC`                                          | `api/cron/sweep-expired-turns.ts` (expired-turn sweep)        |
| `games`         | `phase ASC, reviewDeadline ASC`                                         | `api/cron/resolve-expired-disputes.ts` (expired-review sweep) |
| `games`         | `status ASC, turnNumber ASC, createdAt ASC`                             | Oldest-active-game ordering                                   |
| `notifications` | `gameId ASC, type ASC`                                                  | Per-game notification dedupe lookup                           |

If you add sorting or additional filters, add the index in `firestore.indexes.json` (or the Firebase Console → Firestore → Indexes tab) before deploying — Firestore logs an error with a direct link to create a missing one.
