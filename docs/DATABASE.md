...
| `email`                | `string`    | Copied from Firebase Auth at profile creation (used for notifications + account-deletion flows). Kept in sync via profile updates; do not use it for authoritative email lookups — prefer the Firebase Auth record |
...
| `wins`                 | `number`    | Lifetime wins (maintained by `onGameUpdated` Cloud Function)          |
| `losses`               | `number`    | Lifetime losses (maintained by `onGameUpdated` Cloud Function)        |
| `lastStatsGameId`      | `string`    | Idempotency key for the stats updater                                  |
...
games/{gameId} ──────────────────────────  Storage: games/{gameId}/turn-{N}/{role}.{webm|mp4}
...
- **Path:** `games/{gameId}/turn-{turnNumber}/{role}.{webm|mp4}`
  - `.webm` when the video was recorded by the web build (`MediaRecorder`).
  - `.mp4` when the video was recorded by the Capacitor iOS/Android shell (native camera).
...
| Key                          | Value                                                |
| ---------------------------- | ---------------------------------------------------- |
| `contentType`                | `"video/webm"` (web) or `"video/mp4"` (native)       |
...
The storage rules allowlist file extensions via the regex `(set|match)\.(webm|mp4)` and require the `contentType` to match the extension — a `.webm` upload with `content-type: video/mp4` (or vice versa) is rejected.
