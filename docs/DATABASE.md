...
| `phase`                | `string`                                         | `"setting"` \| `"matching"` \| `"setReview"` \| `"disputable"` \| `"pendingReview"` \| `"communityReview"`. `setReview`/`disputable` only occur on judge-active games; `pendingReview`/`communityReview` only on honor-system games (see the phase table below)                                    |
...
| `statsApplied`         | `boolean?`                                       | **Absent until** the stats close-out Cloud Function runs. Set to `true` transactionally by that function (Admin SDK) when the game reaches `complete`/`forfeit`, recording that both players' `wins`/`losses` have been applied. Acts as the server-side idempotency guard against double-counting |
...
| `reviewFor`            | `string \| null`?                                | Honor-system mirror of `judgeReviewFor`: the claimer (matcher) whose "landed" claim is under review in `pendingReview`/`communityReview`. Server-cleared when the review resolves                                                                                                                  |
| `reviewDeadline`       | `Timestamp \| null`?                             | Deadline of the current review phase (accept window in `pendingReview`, vote window in `communityReview`). **Deliberately distinct from `turnDeadline`** so the turn-forfeit sweep never touches a frozen game                                                                                     |
| `createdAt`            | `Timestamp \| null`                              | Server timestamp at game creation                                                                                                                                                                                                                                                                  |
| `updatedAt`            | `Timestamp \| null`                              | Server timestamp on every write                                                                                                                                                                                                                                                                    |

> **Naming note:** The user-facing copy says "referee" everywhere, but the schema keeps the original `judge*` field names to avoid a Firestore migration for in-flight games. See the v1.1.0 entries in [`CHANGELOG.md`](../CHANGELOG.md) for the full rationale.

**The six phases**

| `phase`           | Path       | Meaning                                                                                     |
| ----------------- | ---------- | ------------------------------------------------------------------------------------------- |
| `setting`         | both       | Setter must name and record a trick                                                         |
| `matching`        | both       | Matcher must attempt it (or "Call BS", if a judge is active)                                |
| `setReview`       | judge only | Judge rules on a "Call BS" against the setter's video                                       |
| `disputable`      | judge only | Judge rules on the matcher's "landed" claim                                                 |
| `pendingReview`   | honor only | Matcher claimed landed; the **setter** has 24 h to accept or dispute. **Freezes the game**  |
| `communityReview` | honor only | The setter disputed; the **community** has 24 h to vote `land`/`bail`. **Freezes the game** |

"Freezes" is literal: while in `pendingReview`/`communityReview`, `currentSetter`, `currentTurn`, `turnNumber`, letters, and `turnHistory` are all pinned, and no `firestore.rules` update branch matches an ordinary turn write. The review clock runs on `reviewDeadline`, not `turnDeadline`, so `api/cron/sweep-expired-turns.ts` skips these games entirely — `api/cron/resolve-expired-disputes.ts` closes them out instead. See [DISPUTE_BINDING_DESIGN.md](DISPUTE_BINDING_DESIGN.md).

**Query patterns:**

- Lobby: three parallel queries — `where("player1Uid", "==", uid)`, `where("player2Uid", "==", uid)` and `where("judgeId", "==", uid)`, each `limit(20)` by default
- Gameplay: single-document listener — `onSnapshot(doc("games/{gameId}"))`
- The three lobby listeners are single-field equality filters, which Firestore indexes automatically. Other `games` queries **do** need composite indexes — five of the twelve declared in `firestore.indexes.json` are on `games` (see §Firestore Indexes)
...
- Phase transitions are validated explicitly (setting↔matching, judge-only setReview/disputable, honor-only pendingReview/communityReview)
...
In-app pokes sent to an idle opponent. No Cloud Function delivers them (the only approved function is the stats close-out — see §`games`) — a nudge is delivered in-app only: the recipient's `subscribeToNudges` listener (wired through `GameNotificationWatcher`) surfaces it as a toast the next time they have the app open. It does NOT wake an offline device. The push drain (`api/cron/drain-push-dispatch.ts`) only consumes `/push_dispatch` (not `/nudges`), and the `push_dispatch` rules whitelist rejects a `"nudge"` type, so routing nudges to OS push would be a rules + product change, not a service-only one. (See `src/services/nudge.ts`.)
...
| `delivered`      | `boolean`   | Always `false` on create. No process flips it — delivery is in-app via the recipient's `subscribeToNudges` listener, not a write-back. The field is a forward-compat placeholder |
...
- Updates are blocked entirely (`allow update: if false`) — nothing marks `delivered`; only the Admin SDK could bypass rules, but no process does
- Deletes are blocked
...
Reserved for server-written billing-alert records. No client access. Nothing writes to this collection today — the deployed stats close-out function does not touch it — so the rules continue to lock it down against any future use.
...
Indexes: `(moderationStatus ASC, createdAt DESC, __name__ DESC)` backs the chronological "New" feed, and `(moderationStatus ASC, upvoteCount DESC, createdAt DESC, __name__ DESC)` backs the default upvote-ranked "Top" feed. Both are declared in `firestore.indexes.json`; `clips.feed.ts` falls back to chronological if the composite index is missing.

The `clips/{clipId}/comments/{commentId}` subcollection holds per-clip comments (signed-in readable, author-deletable).

---

### `disputes/{disputeId}` and `disputeVotes/{voteId}`

Community dispute system — the crowd-judged path that runs alongside the referee. A dispute is raised by the **setter** of a turn against a matcher's "landed" claim; the document id is deterministic (`${gameId}_${turnNumber}`), which the create rule enforces so retries stay idempotent. Reads are open to any signed-in user while `status == "open"`; once resolved, only the setter, the matcher, and users who voted can read it.

`disputeVotes/{voteId}` records one verdict per user per dispute, id-shaped `${uid}_${disputeId}`. Verdicts are `"land"` or `"bail"`, require a verified email (throwaway accounts can't brigade), must target an existing dispute whose `status` is still `"open"`, and you can never vote on your own trick — the rule is the authoritative check, the client's `canVote` flag is only a UI affordance.

Expired disputes are closed out server-side by `api/cron/resolve-expired-disputes.ts` (every 15 min). See [DISPUTE_BINDING_DESIGN.md](DISPUTE_BINDING_DESIGN.md).

---

### `reports/{reportId}` and `reports_limits/{limitId}`

The moderation queue. Any signed-in user can file a report; the reporter can read their own (for the duplicate check in the UI) and an admin can read all of them. **The reported user can never read a report against them** — that would be a retaliation vector. Self-reports are blocked. `gameId` is mandatory for game-sourced reports but is `null` for user-uploaded clips, which must instead identify the target clip, so a null can never produce an unattributable report.

`reports_limits/{reporterUid}_{reportedUid}` enforces a 1-hour cooldown per reporter/target pair, with `lastSentAt` pinned to server time so a client can't back-date the cooldown anchor.

---

### `bans/{uid}`

Admin-issued account bans. The subject may `get` their own ban so the client can explain a rejected write; `list` is admin-only, because a public listing would be a roster of banned accounts. Creates are admin-only and **never against yourself** — the same four-eyes shape as the Verified-Pro grant, so a single compromised admin token can't ban a rival moderator and then clear its own record. Fields are pinned to `bannedBy` / `bannedAt` / `reason`.

Most write rules elsewhere in `firestore.rules` call `notBanned()`, which reads this collection.

---

### `pushTargets/{uid}`, `push_dispatch/{dispatchId}`, `push_dispatch_limits/{limitId}`

The push delivery pipeline.

- **`pushTargets/{uid}`** — the cross-user-readable mirror of a user's FCM tokens (≤10, `updatedAt` pinned to server time). The canonical private copy stays on `users/{uid}/private/profile.fcmTokens`; this mirror exists so a sender can address a device without reading the recipient's private doc. Written by `src/services/fcm.ts` and `pushNotifications.ts`.
- **`push_dispatch/{dispatchId}`** — the outbound queue. A client enqueues a dispatch (sender must be a participant, recipient can't be self); `api/cron/drain-push-dispatch.ts` drains it via the Admin SDK every 5 minutes and sends through FCM. Readable only by the sender and the recipient.
- **`push_dispatch_limits/{limitId}`** — per `(sender, recipient, game, type)` cooldown, deterministic id, `lastSentAt` server-pinned. Its accepted `type` allowlist must stay in lockstep with the `push_dispatch` create rule or that push type dies silently.

Accepted types across both: `your_turn`, `new_challenge`, `game_won`, `game_lost`, `judge_invite`, `nudge`. `nudge` is push-only — it has no `/notifications` feed entry.

---

### `notification_limits/{limitId}`

Server-side rate limiting for in-app notifications. Id is `${senderUid}_${gameId}_${type}` and the rule enforces a 5-second cooldown between writes, with `lastSentAt` pinned to `request.time` so a client can't seed a stale timestamp and defeat the gate. Write-only from the client's perspective. Same `type` allowlist as the push pipeline.

---

### `users/{uid}/blocked_users/{blockedUid}`

Owner-only block list. The document id **is** the blocked uid, and the rule enforces that the stored `blockedUid` field matches it. Self-blocks are rejected. Immutable once created — unblocking is a delete.

---

### `users/{uid}/achievements/{achievementId}` and `users/{uid}/locker/{itemId}`

Public reputation, privileged writes.

- **`achievements`** — earned badges, signed-in readable (they render on `/player/:uid`). Creates are admin-only with an exactly-pinned shape (`earnedAt` server-stamped, `reason` ≤200 chars, no extra keys). A badge is never edited — a wrong award is revoked and re-issued, which keeps `earnedAt` honest. The owner keeps `delete` so the GDPR erasure batch in `src/services/users.ts` can wipe them atomically with the parent doc.
- **`locker`** — Hubba Locker items (economy Phase A). Same model: signed-in readable, admin-only create with a pinned key set (`type`, `brand`, `name`, `imageUrl`, `rarity`, `acquiredAt`, `provenance`), all strings length-capped and `acquiredAt` server-stamped. A client that can mint gear has no economy.
...
games/{gameId} ──────────────────────────  Storage: games/{gameId}/turn-{N}/{role}-{uid}.{webm|mp4}
...
- **Game videos:** `games/{gameId}/turn-{turnNumber}/{role}-{uploaderUid}.{webm|mp4}`
- **Avatars:** `users/{uid}/avatar.{webp|jpeg|png}` — owner-only create/delete, 1 KB – 2 MB, `update: if false`
- **User-posted clips:** `userClips/{uid}/{clipId}.{webm|mp4}` — feed uploads with no backing game; the uid prefix is what makes the object ownable

The uid suffix on game videos is load-bearing: `storage.rules` pins the filename to `{role}-{request.auth.uid}.{ext}` so no account can pre-occupy another player's upload path. `read` and `delete` stay filename-agnostic so objects written under the pre-uid scheme (`set.webm` / `match.mp4`) remain playable and deletable.
...
| Videos               | Deleted eagerly with non-active games during account deletion (`deleteGameVideos`). Otherwise each upload is stamped with a `retainUntil` metadata hint 90 days out (`src/services/storage.ts`), and a Storage lifecycle rule purges objects past that window |
...
**Twelve** composite indexes are declared in `firestore.indexes.json`. The real-time `subscribeToMyGames` listeners use single-field equality filters (`player1Uid == uid`, `player2Uid == uid`, `judgeId == uid`) that Firestore indexes automatically, but the one-time/history and feed queries below add ordering or extra filters that need composite indexes:

| Collection      | Fields                                                                  | Backs                                                        |
| --------------- | ----------------------------------------------------------------------- | ------------------------------------------------------------ |
| `spots`         | `isActive ASC, latitude ASC`                                            | Active-spots map query                                       |
| `clips`         | `moderationStatus ASC, createdAt DESC, __name__ DESC`                   | Feed "new" sort (chronological)                              |
| `clips`         | `moderationStatus ASC, upvoteCount DESC, createdAt DESC, __name__ DESC` | Feed "top" sort (vote-ranked, default — see `clips.feed.ts`) |
| `notifications` | `recipientUid ASC, read ASC, createdAt DESC`                            | Unread-notifications query                                   |
| `games`         | `player1Uid ASC, status ASC, updatedAt DESC`                            | `fetchPlayerCompletedGames` (player1 side)                   |
| `games`         | `player2Uid ASC, status ASC, updatedAt DESC`                            | `fetchPlayerCompletedGames` (player2 side)                   |
| `nudges`        | `recipientUid ASC, createdAt DESC`                                      | Recipient's nudge query                                      |
| `disputes`      | `status ASC, moderationStatus ASC, createdAt DESC, __name__ DESC`       | Community dispute feed                                       |
| `games`         | `status ASC, turnDeadline ASC`                                          | `api/cron/sweep-expired-turns.ts` expired-turn sweep         |
| `games`         | `phase ASC, reviewDeadline ASC`                                         | `api/cron/resolve-expired-disputes.ts` (both frozen phases)  |
| `games`         | `status ASC, turnNumber ASC, createdAt ASC`                             | Cron reminder / backstop passes                              |
| `notifications` | `gameId ASC, type ASC`                                                  | Per-game notification idempotency checks                     |

If you add sorting or additional filters, declare the index in `firestore.indexes.json` — it deploys automatically via `.github/workflows/firebase-rules-deploy.yml` on push to `main`. Do **not** hand-create indexes in the Firebase Console; console-only indexes are invisible to the repo and will drift. Firestore logs an error with a direct creation link when one is missing.
