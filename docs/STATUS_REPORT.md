...
| Vote-driven clip ranking       | **Done**     | `fetchClipsFeed()` in `src/services/clips.feed.ts` defaults to the `'top'` sort — `upvoteCount desc`, then `createdAt desc`, then `__name__ desc` — backed by the `(moderationStatus, upvoteCount, createdAt, __name__)` composite index in `firestore.indexes.json`. A session-scoped circuit breaker falls back to the `'new'` (chronological) sort if that index is still building |
