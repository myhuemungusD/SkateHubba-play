...
| Auto-forfeit on expiry         | **Partial (client-triggered only)** | `forfeitExpiredTurn` in `src/services/games.turns.ts` fires when any client opens the app and observes an expired turn; there is no server-side sweep (the `functions/` `checkExpiredTurns` cron was removed), so a game can sit stale while nobody opens it — see P0-SECURITY-AUDIT.md and CHARTER.md §9.2 |
...
| Push notification **delivery**   | **Needs ops** | `api/cron/drain-push-dispatch.ts` + `.github/workflows/drain-push-dispatch.yml`. The `firestore-send-fcm` extension this depended on does not exist, so no push was ever delivered; the drain endpoint replaces it. Unit-tested but never run against production FCM — requires `VITE_FIREBASE_VAPID_KEY` set in Vercel (without it no device registers a token) and the `CRON_SECRET` repo secret |
