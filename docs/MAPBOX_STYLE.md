...
5. **Keep the Mapbox attribution / wordmark intact.** Mapbox ToS requires
   attribution; the app restores the attribution control in `SpotMap.tsx`.
...
- On a preview deployment, open `/map` and confirm the basemap renders with the
  ...
  `dark-v11`.
- Check the browser console — there should be **no** `[mapbox] Ignoring invalid
VITE_MAPBOX_STYLE_URL` warning. If you see it, the env value isn't a valid
  `mapbox://styles/` URI or `https://` URL.
