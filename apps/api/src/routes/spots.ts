...
router.get("/bounds", getLimiter, async (req, res) => {
  try {
    ...
    // Bounding-box overlap against the GiST-indexed `geom` generated column
    // (see migration 0002). The `&&` operator is the PostGIS bbox-overlap
    // operator and is strictly cheaper than ST_Intersects for viewport
    // queries — it is the pure, index-only bounding-box test.
    const rows = await db
      .select()
      .from(spots)
      .where(
        and(
          sql`geom && ST_MakeEnvelope(${west}, ${south}, ${east}, ${north}, 4326)::geography`,
...
router.get("/:id", getLimiter, async (req, res) => {
  try {
    ...
    const rows = await db.select().from(spots).where(eq(spots.id, req.params.id)).limit(1);
...
router.post("/:id/comments", postLimiter, authMiddleware, async (req, res) => {
  try {
    ...
    const spotRows = await db.select({ id: spots.id }).from(spots).where(eq(spots.id, req.params.id)).limit(1);
