...
export async function clearFirestore(): Promise<void> {
  const res = await fetch(
    `${FS}/emulator/v1/projects/${PROJECT_ID}/databases/${DB_NAME}/documents`,
    { method: "DELETE" },
  );
...
export async function createUser(email: string, password: string): Promise<CreatedUser> {
  const res = await fetch(
    `${AUTH}/identitytoolkit/v3/relyingparty/signupNewUser?key=${API_KEY}`,
    {
      method: "POST",
...
export async function verifyEmail(email: string): Promise<void> {
  ...
  const codesRes = await fetch(
    `${AUTH}/emulator/v1/projects/${PROJECT_ID}/oobCodes`,
  );
  ...
  const applyRes = await fetch(
    `${AUTH}/identitytoolkit/v3/relyingparty/setAccountInfo?key=${API_KEY}`,
    {
      method: "POST",
      headers: { "Content-Type": "application/json" },
...
export async function writeDoc(
  ...
  const fields = Object.fromEntries(
    Object.entries(data).map(([k, v]) => [k, toFsValue(v)]),
  );
