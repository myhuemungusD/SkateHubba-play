 ...
 */
export function TopNewToggle({ sort, onChange }: TopNewToggleProps) {
  return (
    <div
      role="group"
    ...
    >
      <ToggleButton label="Top" pressed={sort === "top"} onClick={() => onChange("top")} />
      <ToggleButton label="New" pressed={sort === "new"} onClick={() => onChange("new")} />
    </div>
  );
}
...
function ToggleButton({ label, pressed, onClick }: { label: string; pressed: boolean; onClick: () => void }) {
  return (
    <button
      ...
      className={`min-h-[44px] inline-flex items-center justify-center rounded-lg px-3.5 font-display text-[11px] tracking-[0.2em] transition-all duration-300 ease-smooth focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-brand-orange active:scale-[0.97] ${
        pressed
