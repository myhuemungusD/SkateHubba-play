...
describe("ClipsFeed", () => {
  ...
  it("still rolls back cleanly when a sort-toggle re-fetch interleaves with an in-flight upvote rejection", async () => {
    ...
    // id is in upvotingIds), so the catch sees our optimistic object still
    // at the key and is safe to roll back. The reference-equality guard in
    // the catch keeps this correct even if a future change lets hydration
    // write through — a fresh server snapshot at that key would fail the
    // ref check and the catch would leave the authoritative value alone.
    const user = userEvent.setup();
