...
export function Toast({ notification, onDismiss }: { notification: AppNotification; onDismiss: (id: string) => void }) {
  ...
  return (
    // No role/aria-live here — it lives on the always-mounted ToastContainer
    // wrapper instead. A screen reader only reliably announces a MUTATION
    // inside an existing live region; a brand-new element born with its
    // text already set (which is what mounting a Toast fresh looks like)
    // is not guaranteed to be picked up.
    <div
      ref={rootRef}
      data-testid="toast"
