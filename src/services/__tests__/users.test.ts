...
describe("users service", () => {
  ...
  describe("applyTrickLanded", () => {
    ...
    it("treats a missing tricksLandedThisGame field as 0 (covers ?? fallback)", async () => {
      // Profile predates the per-game counter — `tricksLandedThisGame`
      // is not present in the doc. The function must treat it as 0 and
      // stage the +1.
      const { tx, update } = makeTx({
        exists: () => true,
        data: () => ({ tricksLanded: 4 }),
      });
      const res = await applyTrickLanded(tx, "u1", "game-fresh");
      expect(res.stagedWrite).toBe(true);
  ...
  describe("backfillStatsIfNeeded", () => {
    ...
    function buildWinDocs(count: number) {
      return Array.from({ length: count }, (_, i) => gameDoc({ id: `g${i}`, winner: "u1", ageMs: i }));
    }

    /** Stub runTransaction to invoke its callback with a no-op tx.
     *  The tx exposes both `get` (returns a doc with no
     *  `statsBackfilledAt` so the in-tx idempotency check passes —
     *  audit A2-BLOCKER-3) and `update`. Tests that need the
     *  already-backfilled in-tx branch override `getResult`. */
    function stubTx(
      updateSpy: (...args: unknown[]) => unknown = () => undefined,
      ...
      mockRunTransaction.mockImplementationOnce(
        async (
          _db: unknown,
          fn: (tx: { update: typeof updateSpy; get: () => Promise<typeof getResult> }) => Promise<void>,
        ) => {
          await fn({
            update: updateSpy,
            get: async () => getResult,
    ...
    /**
     * Drive the function through the outside-tx fast-path with a
     * caller-supplied `tx.get` payload, returning the spy that captures
     * any staged update. Used by the in-tx idempotency tests below
     * (audit A2-BLOCKER-3).
     */
    async function runWithInTxSnapshot(txGetResult: {
      exists: () => boolean;
      data: () => unknown;
    }): Promise<{ updateSpy: ReturnType<typeof vi.fn>; result: BackfillStatsResult }> {
      mockGetDoc.mockResolvedValueOnce({
        exists: () => true,
        data: () => ({ uid: "u1", statsBackfilledAt: null }),
      });
      ...
      mockRunTransaction.mockImplementationOnce(
        async (
          _db: unknown,
          fn: (tx: { update: typeof updateSpy; get: () => Promise<typeof txGetResult> }) => Promise<void>,
        ) => {
          await fn({ update: updateSpy, get: async () => txGetResult });
        },
      );
      const result = await backfillStatsIfNeeded("u1");
      return { updateSpy, result };
    }

    it("treats a non-existent in-tx snapshot as not-yet-backfilled (covers !exists branch)", async () => {
      const { updateSpy, result } = await runWithInTxSnapshot({
        exists: () => false,
        data: () => ({}),
      });
      expect(result).toEqual({ backfilled: true, partial: false });
    ...
    it("returns without staging an update when statsBackfilledAt is set INSIDE the transaction (TOCTOU defence — audit A2-BLOCKER-3)", async () => {
      const { updateSpy, result } = await runWithInTxSnapshot({
        exists: () => true,
        data: () => ({ uid: "u1", statsBackfilledAt: { seconds: 42 } }),
      });
      expect(result).toEqual({ backfilled: false, partial: false });
