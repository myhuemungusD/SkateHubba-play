 ...
 * the TODO below — so callers passing 0 today see no level change.
 */
export async function applyGameOutcome(
  ...
  try {
    ...
    // TODO(PR-E): apply xpDelta and recompute `level` via LEVEL_THRESHOLDS
    // lookup in src/constants/xp.ts. PR-A1 callers always pass 0, so we
