...
export async function backfillStatsIfNeeded(uid: string): Promise<BackfillStatsResult> {
  ...
  try {
    ...
    // Idempotency check INSIDE the transaction (audit A2-BLOCKER-3 —
    // TOCTOU). Two concurrent calls (e.g. two open tabs) would both
    // pass the outside-tx fast-path above, both compute counters, and
    // both stage the update — doubling every counter. Re-reading the
    // doc inside the transaction guarantees only one writer commits.
    let alreadyBackfilled = false;
    await runTransaction(db, async (tx) => {
      const txSnap = await tx.get(userRef);
      const txData = txSnap.exists() ? (txSnap.data() as Partial<UserProfile> & { statsBackfilledAt?: unknown }) : null;
      if (txData?.statsBackfilledAt != null) {
    ...
    if (alreadyBackfilled) {
      addBreadcrumb({
        category: "stats",
        message: "backfillStatsIfNeeded.skipped_already_backfilled_intx",
        data: { uid },
      });
      return { backfilled: false, partial: false };
    }
 ...
 */
function computeBackfillCounters(
  uid: string,
  games: BackfillGameLike[],
): {
  gamesWon: number;
  gamesLost: number;
  gamesForfeited: number;
