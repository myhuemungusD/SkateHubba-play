...
describe("users/{uid} — sensitive fields forbidden at top level on UPDATE", () => {
  it("attack: CANNOT update users/{uid} to add email at top level", async () => {
    await assertFails(
      setDoc(doc(asOwner().firestore(), "users", OWNER_UID), publicUserUpdate({ email: "alice@example.com" })),
    );
  });

  it("attack: CANNOT update users/{uid} to add emailVerified at top level", async () => {
    await assertFails(
      setDoc(doc(asOwner().firestore(), "users", OWNER_UID), publicUserUpdate({ emailVerified: true })),
    );
  });

  it("attack: CANNOT update users/{uid} to add dob at top level", async () => {
    await assertFails(setDoc(doc(asOwner().firestore(), "users", OWNER_UID), publicUserUpdate({ dob: "2000-01-15" })));
  });

  it("attack: CANNOT update users/{uid} to add parentalConsent at top level", async () => {
    await assertFails(
      setDoc(doc(asOwner().firestore(), "users", OWNER_UID), publicUserUpdate({ parentalConsent: true })),
    );
  });

  it("attack: CANNOT update users/{uid} to add fcmTokens at top level", async () => {
    await assertFails(
      setDoc(doc(asOwner().firestore(), "users", OWNER_UID), publicUserUpdate({ fcmTokens: ["tok-1"] })),
    );
  });

  it("legitimate: owner CAN update stats on users/{uid} without sensitive fields", async () => {
    await assertSucceeds(setDoc(doc(asOwner().firestore(), "users", OWNER_UID), publicUserUpdate({ wins: 1 })));
  });
});
