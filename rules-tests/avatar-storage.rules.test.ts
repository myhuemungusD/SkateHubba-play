...
describe("avatar storage rules — owner can upload", () => {
  it("owner CAN upload a valid webp at users/{uid}/avatar.webp", async () => {
    await assertSucceeds(asOwner().storage().ref(ownerAvatarPath()).put(smallPayload(), { contentType: "image/webp" }));
  });

  it("owner CAN upload jpeg + png variants", async () => {
    await assertSucceeds(
      asOwner().storage().ref(ownerAvatarPath("jpeg")).put(smallPayload(), { contentType: "image/jpeg" }),
    );
    await assertSucceeds(
      asOwner().storage().ref(ownerAvatarPath("png")).put(smallPayload(), { contentType: "image/png" }),
    );
  });
});
...
describe("avatar storage rules — strangers + anonymous denied", () => {
  it("stranger CANNOT upload at another user's path", async () => {
    await assertFails(asStranger().storage().ref(ownerAvatarPath()).put(smallPayload(), { contentType: "image/webp" }));
  });
...
describe("avatar storage rules — content-type pinning", () => {
  it("denies a PDF upload at the avatar path", async () => {
    await assertFails(
      asOwner().storage().ref(ownerAvatarPath()).put(smallPayload(), { contentType: "application/pdf" }),
    );
  });

  it("denies an extension outside the allowlist (gif)", async () => {
    await assertFails(
      asOwner().storage().ref(`users/${OWNER_UID}/avatar.gif`).put(smallPayload(), { contentType: "image/gif" }),
    );
  });

  it("denies a mismatched extension/content-type pair (audit B-ISSUE-3)", async () => {
    // .webp filename with a PNG content-type — the rule must cross-
    // validate the two and reject so a client cannot lie about the
    // payload format.
    await assertFails(
      asOwner().storage().ref(ownerAvatarPath("webp")).put(smallPayload(), { contentType: "image/png" }),
    );
    await assertFails(
      asOwner().storage().ref(ownerAvatarPath("png")).put(smallPayload(), { contentType: "image/jpeg" }),
    );
  });
});
...
describe("avatar storage rules — size cap", () => {
  it("denies a 3 MB upload (>2 MB cap)", async () => {
    await assertFails(
      asOwner().storage().ref(ownerAvatarPath()).put(oversizedPayload(), { contentType: "image/webp" }),
    );
  });
});
